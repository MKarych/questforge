...
## 21. AI-помощник в редакторе

### 21.1. Кнопка "✨ Доработать AI"

- Место: панель инструментов (рядом с "Сохранить", "Превью", "Тест").
- При клике: открывается модалка с полем для промпта.

### 21.2. Модалка AI-доработки

```text
┌─────────────────────────────────────────────────────────────┐
│  ✨ AI-доработка сценария                                  │
│  ──────────────────────────────────────────────────────────│
│                                                             │
│  Что сделать?                                               │
│  ┌─────────────────────────────────────────────────────────┐│
│  │  Сделай задания сложнее                              ││
│  │  Добавь подсказки                                    ││
│  │  Упрости логику                                      ││
│  └─────────────────────────────────────────────────────────┘│
│                                                             │
│  [   🧠 Сгенерировать   ]                                  │
│                                                             │
│  🔑 Используется OpenRouter AI (бесплатно, 3 генерации/день)│
└─────────────────────────────────────────────────────────────┘
21.3. Лимиты
text
Free: 3 генерации в день
PRO: 50 генераций в день
Business: 500 генераций в день
22. Компактная палитра блоков
22.1. Текущая проблема
Палитра — длинный список. Чтобы добраться до нижних блоков, надо скроллить.

22.2. Решение
Сетка 2×6 вместо списка.

Группировка по категориям (Базовые, Задания, Логика).

Поиск остаётся.

Ширина: 180px (вместо 220px).

23. Кнопка "Очистить всё"
23.1. Место
Панель инструментов (рядом с "Сохранить").

Иконка: 🗑️.

23.2. Поведение
text
1. Нажатие → модалка подтверждения.
2. Текст: "Вы уверены, что хотите удалить все блоки? Это действие нельзя отменить."
3. Кнопки: "Да, удалить" / "Отмена".
4. После подтверждения: удаляются все сцены и связи, остаются START и FINISH.
23.3. Безопасность
Undo (Ctrl+Z) работает.

Автосохранение не срабатывает мгновенно.
