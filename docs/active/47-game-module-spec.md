...
interface Game {
  ...
  scenarioId: string;               // ID сценария (ОБЯЗАТЕЛЬНО при создании)
...
interface GameSettings {
  ...
  scenarioId: string;               // ID сценария (ОБЯЗАТЕЛЬНО при создании игры)
...
PUBLISHED	Видна в каталоге	Видна в каталоге, можно регистрироваться
...
7. Сценарий ДОЛЖЕН БЫТЬ указан (обязательное поле).
8. Сценарий должен быть опубликован (isPublished = true).
9. Город и описание обязательны.
10. Организатор должен иметь роль ORGANIZER или ADMIN.
11. Нельзя создать игру без сценария.
...
11. Сценарий обязателен при создании игры.
15. Domain Events
typescript
