...
import {
  DomainEventBus,
  UserDomainEventType,
} from './domain/user-domain-events';
...
export class UsersService {
  ...
  // ============================================================
  async updateProfile(userId: string, dto: any, ip?: string, userAgent?: string) {
    const user = await this.prisma.user.findFirst({
      where: { id: userId, deletedAt: null },
    });
    ...
    const oldProfile = { ...((u.profile as any) || {}) };
    const oldSettings = { ...((u.settings as any) || {}) };
    ...
    await this.eventBus.publish({
      type: UserDomainEventType.ProfileUpdated,
      userId,
    ...
    if (dto.privacySettings) {
      await this.eventBus.publish({
        type: UserDomainEventType.ProfilePrivacyChanged,
        userId,
  ...
  async updateAvatar(userId: string, avatarUrl: string, ip?: string, userAgent?: string) {
    ...
    const u: any = user;
    const profile = { ...((u.profile as any) || {}), avatar: avatarUrl };
    ...
    await this.createAuditLog(userId, 'avatar.changed', 'User', userId, {
      old: { avatar: (u.profile as any)?.avatar || u.avatarUrl },
      new: { avatar: avatarUrl },
    }, ip, userAgent);

    await this.eventBus.publish({
      type: UserDomainEventType.AvatarChanged,
      userId,
      payload: { avatarUrl },
      timestamp: new Date(),
      version: 2,
  ...
  // ============================================================
  async deleteAvatar(userId: string, ip?: string, userAgent?: string) {
    const user = await this.prisma.user.findFirst({
      where: { id: userId, deletedAt: null },
    });
    ...
    const u: any = user;
    const profile = { ...((u.profile as any) || {}), avatar: null };
    ...
    await this.eventBus.publish({
      type: UserDomainEventType.AvatarDeleted,
      userId,
      payload: {},
      timestamp: new Date(),
      version: 2,
  ...
  // ============================================================
  async deleteUser(userId: string, ip?: string, userAgent?: string) {
    const user = await this.prisma.user.findFirst({
      where: { id: userId, deletedAt: null },
    });
    ...
    await this.eventBus.publish({
      type: UserDomainEventType.UserDeleted,
      userId,
      payload: {},
      timestamp: new Date(),
      version: 2,
  ...
  async followUser(followerId: string, followingId: string) {
    ...
    await this.eventBus.publish({
      type: UserDomainEventType.FollowCreated,
      userId: followerId,
      payload: { followingId },
      timestamp: new Date(),
      version: 2,
  ...
  async unfollowUser(followerId: string, followingId: string) {
    ...
    await this.eventBus.publish({
      type: UserDomainEventType.FollowDeleted,
      userId: followerId,
      payload: { followingId },
      timestamp: new Date(),
      version: 2,
  ...
  async getFollowers(userId: string, limit = 20, offset = 0) {
    const [items, total] = await Promise.all([
      ...
      (this.prisma as any).follow.count({ where: { followingId: userId } }),
    ]);
  ...
  async getFollowing(userId: string, limit = 20, offset = 0) {
    const [items, total] = await Promise.all([
      ...
      (this.prisma as any).follow.count({ where: { followerId: userId } }),
    ]);
  ...
  async addFavorite(userId: string, category: 'games' | 'scenarios' | 'authors', itemId: string) {
    const user = await this.prisma.user.findFirst({
      where: { id: userId, deletedAt: null },
    });
    ...
    await this.eventBus.publish({
      type: UserDomainEventType.FavoritesUpdated,
      userId,
  ...
  async removeFavorite(userId: string, category: 'games' | 'scenarios' | 'authors', itemId: string) {
    const user = await this.prisma.user.findFirst({
      where: { id: userId, deletedAt: null },
    });
    ...
    if (!favorites[category]) return favorites;
    favorites[category] = favorites[category].filter((id: string) => id !== itemId);
    profile.favorites = favorites;
  ...
  async getActivityFeed(userId: string, limit = 20, offset = 0) {
    const [items, total] = await Promise.all([
      ...
      (this.prisma as any).activityLog.count({ where: { userId } }),
    ]);

    return { items, total };
  }

  async addActivity(userId: string, type: string, payload: Record<string, unknown> = {}) {
  ...
  async getUserScenarios(userId: string, limit = 20, offset = 0) {
    const [items, total] = await Promise.all([
  ...
  async addAchievement(userId: string, achievement: Achievement) {
    ...
    await this.eventBus.publish({
      type: UserDomainEventType.AchievementUnlocked,
      userId,
  ...
  async checkAndAwardAchievements(userId: string) {
    ...
    if (user.scenariosCreated >= 1 && !achievements.some((a: Achievement) => a.type === 'FIRST_SCENARIO')) {
    ...
    if (roles.includes('ORGANIZER') && !achievements.some((a: Achievement) => a.type === 'ORGANIZER')) {
    ...
    if (user._count.captainTeams >= 1 && !achievements.some((a: Achievement) => a.type === 'FIRST_GAME')) {
    ...
    if (user._count.captainTeams >= 10 && !achievements.some((a: Achievement) => a.type === 'TEN_GAMES')) {
    ...
    if (user._count.captainTeams >= 50 && !achievements.some((a: Achievement) => a.type === 'FIFTY_GAMES')) {
    ...
    if (user._count.captainTeams >= 100 && !achievements.some((a: Achievement) => a.type === 'HUNDRED_GAMES')) {
  ...
  async recalculateCapabilities(userId: string) {
    const user = await this.prisma.user.findFirst({
      where: { id: userId },
    ...
    if (user.scenariosCreated >= 1 || roles.includes('ORGANIZER')) capabilities.push('CREATE_SCENARIOS');
    if (roles.includes('ORGANIZER')) capabilities.push('HOST_EVENTS');
    if (roles.includes('ORGANIZER') && user.scenariosCreated >= 1) capabilities.push('SELL_SCENARIOS');
    if (roles.includes('ADMIN') || roles.includes('MODERATOR')) capabilities.push('MODERATE_CONTENT');
    if (user.gamesConducted >= 1) capabilities.push('STREAM_GAME');
  ...
  async calculateUserRating(userId: string) {
    ...
    const rep = { ...((user?.reputationData as any) || {}) };
    rep.rating = rounded;
  ...
  private async createAuditLog(
    ...
    changes: { old?: unknown; new?: unknown }, ip?: string, userAgent?: string,
  ) {
    try {
