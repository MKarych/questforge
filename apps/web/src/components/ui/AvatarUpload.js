...
const ImageCropper_1 = __importDefault(require("./ImageCropper"));
function AvatarUpload({ currentAvatar, onUpload, onDelete }) {
    ...
    // ===== Обрезка завершена =====
    const handleCropComplete = (0, react_1.useCallback)(async (croppedBlob) => {
    ...
    // ===== Отмена обрезки =====
    const handleCropCancel = (0, react_1.useCallback)(() => {
        setShowCropper(false);
        setSelectedFile(null);
    }, []);
    // ===== Удаление аватара =====
    ...
    const handleZoneClick = (0, react_1.useCallback)(() => {
    ...
    return (<>
      ...
      <div className={`
        ...
        <div className={`
          ...
          `}>
          {displayUrl ? (<img src={displayUrl} alt="Avatar" className="w-full h-full object-cover"/>) : (<div className="w-full h-full flex items-center justify-center text-text-secondary">
        ...
        <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
          {currentAvatar && (<button onClick={handleDelete} className="text-sm px-4 py-2 text-error hover:text-error/80 transition-colors">
              Удалить фото
            </button>)}

          {!currentAvatar && (<button onClick={handleZoneClick} className="text-sm px-4 py-2 text-primary hover:text-primary-hover transition-colors">
              Выбрать фото
            </button>)}
        </div>
      ...
      {/* Cropper modal */}
      {showCropper && selectedFile && (<ImageCropper_1.default file={selectedFile} onCrop={handleCropComplete} onCancel={handleCropCancel}/>)}
    </>);
}
