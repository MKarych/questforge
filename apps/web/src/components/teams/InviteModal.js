...
function InviteModal({ teamId, isOpen, onClose, onSuccess }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await (0, client_1.inviteToTeam)(teamId, data);
            onSuccess();
            onClose();
            setUserId('');
            setMessage('');
      ...
      <div className="card max-w-md w-full mx-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" value={userId} onChange={(e) => setUserId(e.target.value)} placeholder="Введите ID пользователя" className="input-field" required/>
          </div>

          <div>
            <label className="label">Сообщение (необязательно)</label>
            <textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Напишите приглашение..." className="input-field" rows={3} maxLength={500}/>
          </div>
...
//# sourceMappingURL=InviteModal.js.map