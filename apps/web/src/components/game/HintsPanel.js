...
exports.default = HintsPanel;
const react_1 = require("react");
function HintsPanel({ hints, unlockedLevels, usedLevels, onUseHint, currentPenalty }) {
    ...
    return (<div className="card">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {availableHints.length > 0 && (<span className="bg-primary text-white text-xs px-2 py-1 rounded-full">
      ...
      {isOpen && (<div className="mt-4 space-y-3">
          {hints.map((hint) => {
                ...
                return (<div key={hint.level} className={`p-3 rounded-lg border-2 transition-all ${isUsed
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  {isUsed ? (<span className="text-success font-bold">✓</span>) : isUnlocked ? (<span className="text-error text-sm font-medium">-{Math.abs(hint.penalty)} очк.</span>) : (<span className="text-text-secondary text-xs">🔒 {hint.unlockedAt}с</span>)}
                </div>

                {isUsed ? (<p className="text-text-secondary text-sm pl-7">{hint.text}</p>) : isUnlocked ? (<button onClick={() => onUseHint(hint)} disabled={isUsed} className="btn-secondary text-sm w-full mt-2">
                    Использовать (-{Math.abs(hint.penalty)} очк.)
                  </button>) : (<p className="text-text-secondary text-sm pl-7 italic">
                    Откроется через {hint.unlockedAt} секунд
                  </p>)}
              </div>);
            })}

          {usedHints.length > 0 && (<div className="text-xs text-text-secondary text-center mt-2">
              Общий штраф за подсказки: <span className="text-error font-bold">{currentPenalty}</span>
            </div>)}
        </div>)}
    </div>);
}
//# sourceMappingURL=HintsPanel.js.map