...
export default function NodeSettings({ node, allNodes, onUpdate, onDelete, onClose }: NodeSettingsProps) {
        ...
        <>
          ...
          <div>
            ...
            <textarea
              ...
              className="input-field text-sm"
              placeholder="Подсказка для игроков (опционально)"
              rows={2}
            />
          </div>
  ...
  const renderPHOTOFields = () => (
    <div className="space-y-4">
      <div>
        ...
        <textarea
          value={data.photoRequirements || ''}
          onChange={(e) => handleChange('photoRequirements', e.target.value)}
          className="input-field text-sm min-h-[60px]"
          placeholder="Опишите, какое фото нужно сделать..."
          rows={2}
        />
      </div>
    </div>
    ...
    <div className="space-y-4">
      <div>
        ...
        <input
          ...
          className="input-field text-sm"
          placeholder="Введите данные, которые будет сканировать QR-код"
        />
      </div>
    </div>
  );
          ...
          {(data.options ?? []).map((option, index) => (
            <div key={index} className="flex gap-2 items-center">
              <input
                ...
                onChange={() => handleChange('correctOption', index)}
              ...
              <button
                ...
                onClick={() => {
                  ...
                  handleChange('options', newOptions);
                  if (newOptions.length > 0) handleChange('correctOption', newCorrect);
                }}
      ...
      <div>
        ...
        <div className="space-y-2">
          {(data.branches ?? []).map((branch, index) => (
            <div key={index} className="flex gap-2">
              <input
                ...
                className="input-field text-sm flex-1"
                placeholder={`Метка ветки ${index + 1}`}
              />
              <input
                type="text"
                value={branch.target}
          ...
          <button
            type="button"
            onClick={() =>
              handleChange('branches', [...(data.branches ?? []), { label: 'Далее', target: '' }])
            }
            className="btn-secondary text-sm w-full"
          >
            + Добавить ветку
    ...
    <div className="space-y-4">
      <div>
        ...
        <input
          type="text"
          value={data.npcName || ''}
          onChange={(e) => handleChange('npcName', e.target.value)}
          className="input-field text-sm"
          placeholder="Имя NPC"
        />
      </div>
      <div>
        ...
        <textarea
          value={data.npcDescription || ''}
          onChange={(e) => handleChange('npcDescription', e.target.value)}
          className="input-field text-sm min-h-[60px]"
          placeholder="Внешность, характер персонажа..."
          rows={2}
        />
      </div>
      <div>
            ...
            <div key={index} className="p-3 bg-background/50 rounded border border-border">
              <div className="flex gap-2 mb-2">
                <input
                  ...
                  className="input-field text-sm flex-1"
                  placeholder={`Реплика NPC ${index + 1}`}
                />
                <button
                  type="button"
                  onClick={() => {
