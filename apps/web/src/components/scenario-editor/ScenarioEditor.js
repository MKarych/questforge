...
const NodeSettings_1 = __importDefault(require("./NodeSettings"));
const ScenarioNode_1 = __importDefault(require("./ScenarioNode"));
const nodeTypes = {
    START: ScenarioNode_1.default,
    FINISH: ScenarioNode_1.default,
    TEXT: ScenarioNode_1.default,
    CODE: ScenarioNode_1.default,
    PHOTO: ScenarioNode_1.default,
    GPS: ScenarioNode_1.default,
    QR: ScenarioNode_1.default,
    CHOICE: ScenarioNode_1.default,
    TIMER: ScenarioNode_1.default,
    BRANCH: ScenarioNode_1.default,
    NPC: ScenarioNode_1.default,
};
// Custom edge component with visual feedback and delete button
const ScenarioEdgeComponent = ({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, selected, data, }) => {
    ...
    const { setEdges } = (0, reactflow_1.useReactFlow)();
    const handleDeleteEdge = (0, react_1.useCallback)((e) => {
        e.stopPropagation();
    ...
    return (<>
      ...
      {data?.condition?.label && (<div style={{
                position: 'absolute',
      ...
      <div style={{
            ...
            cursor: 'pointer',
        }} className="edge-delete-btn group-hover:opacity-100" onClick={handleDeleteEdge} title="Удалить соединение">
        <div style={{
            width: 20,
            height: 20,
            borderRadius: '50%',
...
function ScenarioEditorInner({ scenarioName, initialNodes, initialEdges, initialSettings, isPublished = false, onSave, onPublish, onNodesChange: onNodesChangeCallback, onEdgesChange: onEdgesChangeCallback, }) {
    const reactFlowInstance = (0, reactflow_1.useReactFlow)();
    const [name, setName] = (0, react_1.useState)(scenarioName || 'Новый сценарий');
    const [nodes, setNodes, handleNodesChange] = (0, reactflow_1.useNodesState)(initialNodes || []);
    const [edges, setEdges, handleEdgesChange] = (0, reactflow_1.useEdgesState)(initialEdges || []);
    const [selectedNode, setSelectedNode] = (0, react_1.useState)(null);
    const [selectedEdge, setSelectedEdge] = (0, react_1.useState)(null);
    const [validationErrors, setValidationErrors] = (0, react_1.useState)({ valid: true, errors: [] });
    ...
    const [copiedNode, setCopiedNode] = (0, react_1.useState)(null);
    const [currentPreviewNode, setCurrentPreviewNode] = (0, react_1.useState)(null);
    const [showSettings, setShowSettings] = (0, react_1.useState)(false);
    const [isDirty, setIsDirty] = (0, react_1.useState)(false);
    const isEmpty = (0, react_1.useMemo)(() => nodes.length === 0, [nodes]);
    // Game settings
    const [gameSettings, setGameSettings] = (0, react_1.useState)(initialSettings || {
        totalTime: 0,
        defaultPoints: 10,
        defaultPenalty: 0,
        hintLimit: 3,
    ...
    // Test mode state
    const [testCurrentNodeId, setTestCurrentNodeId] = (0, react_1.useState)(null);
    const [testVariables, setTestVariables] = (0, react_1.useState)({});
    const [testScore, setTestScore] = (0, react_1.useState)(0);
    const [testLog, setTestLog] = (0, react_1.useState)([]);
    const [testFinished, setTestFinished] = (0, react_1.useState)(false);
    // Initialize with START and FINISH nodes
    (0, react_1.useEffect)(() => {
        if (nodes.length === 0) {
            const startId = (0, uuid_1.v4)();
            const finishId = (0, uuid_1.v4)();
            setNodes([
                {
                    id: startId,
                    type: 'START',
    ...
    // Save history state
    const saveHistory = (0, react_1.useCallback)((actionType, nodeId, edgeId) => {
        const state = {
            nodes,
            edges,
            action: actionType,
    ...
    const deleteEdge = (0, react_1.useCallback)((edgeId) => {
        saveHistory('disconnect', undefined, edgeId);
        setEdges((eds) => eds.filter((edge) => edge.id !== edgeId));
        setSelectedEdge(null);
    }, [setEdges, saveHistory]);
    // Mark dirty on changes
    const markDirty = (0, react_1.useCallback)(() => {
        setIsDirty(true);
    }, []);
    // Keyboard shortcuts
    (0, react_1.useEffect)(() => {
    ...
    const onDrop = (0, react_1.useCallback)((event) => {
        ...
        const block = scenario_1.BLOCK_TYPES.find((b) => b.type === type);
        if (!block)
            return;
    ...
    const onConnect = (0, react_1.useCallback)((params) => {
        ...
        const defaultConditionType = sourceNode && sourceNode.type && missionTypes.includes(sourceNode.type) ? 'success' : 'always';
        setEdges((eds) => (0, reactflow_1.addEdge)({
    ...
    const onEdgeClick = (0, react_1.useCallback)((_, edge) => {
        setSelectedEdge(edge);
        setSelectedNode(null);
    }, []);
    const onSelectionChange = (0, react_1.useCallback)(({ nodes: selectedNodes, edges: selectedEdges }) => {
        if (selectedNodes.length === 0 && selectedEdges.length === 0) {
            setSelectedNode(null);
            setSelectedEdge(null);
        }
    }, []);
    const updateNodeData = (0, react_1.useCallback)((nodeId, data) => {
    ...
    const pasteNode = (0, react_1.useCallback)((nodeToCopy) => {
        const newNode = {
            ...nodeToCopy,
            id: (0, uuid_1.v4)(),
            position: {
    ...
    // Improved validation
    const validateScenario = (0, react_1.useCallback)(() => {
        const errors = [];
        ...
        if (startNode && hasCycle(startNode.id, new Set(), new Set())) {
            errors.push({
    ...
    (0, react_1.useEffect)(() => {
        const errorNodeIds = new Set(validationErrors.errors
        ...
        setNodes((nds) => nds.map((node) => {
            ...
            const errorMsg = validationErrors.errors.find((e) => e.nodeId === node.id)?.message;
    ...
    const startNodeId = (0, react_1.useMemo)(() => nodes.find((n) => n.type === 'START')?.id || '', [nodes]);
    // Preview handler
    const handlePreviewNode = (nodeId) => {
    ...
    const handleTestAnswer = (0, react_1.useCallback)((answer) => {
        if (!testCurrentNodeId || testFinished)
            return;
        ...
        if (currentNode.type === 'CHOICE') {
            ...
            const matchingEdge = outgoingEdges.find((e) => e.data?.condition?.type === conditionType);
        ...
        if (!nextNodeId) {
            const alwaysEdge = outgoingEdges.find((e) => e.data?.condition?.type === 'always');
    ...
    // Variable management
    const addVariable = (0, react_1.useCallback)(() => {
        const newVar = {
            id: (0, uuid_1.v4)(),
            name: '',
            type: 'number',
    ...
    const updateVariable = (0, react_1.useCallback)((id, field, value) => {
        setGameSettings((prev) => ({
            ...prev,
            variables: prev.variables.map((v) => v.id === id ? { ...v, [field]: value } : v),
        }));
    }, []);
    const removeVariable = (0, react_1.useCallback)((id) => {
        setGameSettings((prev) => ({
            ...prev,
    ...
    return (<div className="flex flex-col h-screen">
      ...
      <div className="h-14 bg-background border-b border-border flex items-center justify-between px-4">
        <div className="flex items-center gap-4">
          <input type="text" value={name} onChange={(e) => {
            ...
            markDirty();
        }} className="text-lg font-bold bg-transparent border-none focus:outline-none focus:ring-2 focus:ring-primary/30 rounded px-2 py-1" placeholder="Название сценария"/>
          {/* Status indicator */}
          <span className={`text-xs font-semibold px-2 py-1 rounded ${isPublished
            ? 'bg-green-500/20 text-green-400 border border-green-500/30'
            : 'bg-gray-500/20 text-gray-400 border border-gray-500/30'}`}>
            {isPublished ? 'PUBLISHED' : 'DRAFT'}
          </span>
          {startNodeId && (<span className="text-xs text-text-secondary bg-primary/10 px-2 py-1 rounded">
        ...
        }} className="btn-secondary text-sm" title="Проверить сценарий (Ctrl+Enter)">
            ✅ Проверить
          </button>
          <button onClick={() => handlePreviewNode(startNodeId)} className="btn-secondary text-sm" title="Превью игры (Ctrl+P)">
            👁 Превью
          </button>
          <button onClick={handleTestStart} className="btn-secondary text-sm" title="Тестирование сценария">
            🎮 Тест
          </button>
          <button onClick={() => setShowSettings(!showSettings)} className="btn-secondary text-sm" title="Настройки игры">
            ⚙️ Настройки
          </button>
          <button onClick={undo} disabled={historyIndex <= 0} className="btn-secondary text-sm disabled:opacity-30" title="Отменить (Ctrl+Z)">
            ↩️ Отменить
          </button>
          <button onClick={redo} disabled={historyIndex >= history.length - 1} className="btn-secondary text-sm disabled:opacity-30" title="Повторить (Ctrl+Shift+Z)">
            ↪️ Повторить
          </button>
          {/* Publish button — only if not published */}
          {!isPublished && onPublish && (<button onClick={onPublish} className="btn-primary text-sm bg-green-600 hover:bg-green-700">
              📢 Опубликовать
            </button>)}
          {/* Save button — disabled if no changes */}
          <button onClick={handleSave} disabled={!isDirty} className={`btn-primary text-sm ${!isDirty ? 'opacity-40 cursor-not-allowed' : ''}`} title={!isDirty ? 'Нет изменений для сохранения' : 'Сохранить (Ctrl+S)'}>
            💾 Сохранить
          </button>
        </div>
      </div>
      ...
      {/* Validation Errors */}
      {!validationErrors.valid && validationErrors.errors.length > 0 && (<div className="bg-error/10 border-b border-error/30 px-4 py-2">
      ...
      <div className="flex-1 flex">
        {/* Block Palette */}
        <BlockPalette_1.default onDragStart={onDragStart}/>

        {/* Canvas */}
        <div className="flex-1 relative" onDrop={onDrop} onDragOver={onDragOver}>
          {/* Empty state */}
          {isEmpty && (<div className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none">
          ...
          <reactflow_1.default nodes={nodes} edges={edges} onNodesChange={(changes) => {
            handleNodesChange(changes);
            onNodesChangeCallback?.();
        }} onEdgesChange={(changes) => {
            handleEdgesChange(changes);
            onEdgesChangeCallback?.();
        }} onConnect={onConnect} onNodeClick={onNodeClick} onEdgeClick={onEdgeClick} onSelectionChange={onSelectionChange} onPaneClick={() => {
            setSelectedNode(null);
            setSelectedEdge(null);
        }} nodeTypes={nodeTypes} edgeTypes={edgeTypes} fitView snapToGrid snapGrid={[15, 15]} className="bg-background" deleteKeyCode={null}>
            <reactflow_1.Background color="#888" gap={20}/>
            <reactflow_1.Controls />
            <reactflow_1.MiniMap nodeStrokeColor={(n) => {
            const block = scenario_1.BLOCK_TYPES.find((b) => b.type === n.type);
        ...
        {/* Node Settings Panel */}
        {selectedNode && (<NodeSettings_1.default node={selectedNode} allNodes={nodes} onUpdate={updateNodeData} onDelete={deleteNode} onClose={() => setSelectedNode(null)}/>)}
      </div>

      {/* Preview Modal — Phone 375x812 */}
      {showPreview && currentPreviewNode && (<div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-background rounded-lg shadow-2xl overflow-hidden flex flex-col">
            ...
            <div className="bg-gray-800 p-4 flex items-center justify-between">
              ...
              <button onClick={() => setShowPreview(false)} className="text-gray-400 hover:text-white text-xl">
                ✕
              </button>
            </div>
            {/* Phone Container */}
            <div className="p-6 bg-gray-900 flex flex-col items-center">
              ...
              <div className="bg-white rounded-[2rem] overflow-hidden shadow-2xl flex flex-col border-4 border-gray-700" style={{ width: 375, height: 812, maxWidth: '100%' }}>
                {/* Phone Notch */}
                <div className="bg-gray-900 h-8 flex items-center justify-center relative">
                  ...
                  <div className="absolute right-4 top-2 text-[10px] text-white font-semibold">
                    {String(nodes.filter((n) => n.type !== 'START' && n.type !== 'FINISH').indexOf(nodes.find((n) => n.id === currentPreviewNode)) + 1)}
                ...
                const taskIndex = taskNodes.indexOf(node);
                ...
                return (<div className="space-y-5">
                        ...
                        {/* Question */}
                        {node.data.question && (<div className="text-gray-700 text-sm leading-relaxed">
                            {node.data.question}
                          </div>)}
                        {/* Hint */}
                        {node.data.hint && (<div className="bg-yellow-50 border border-yellow-200 p-3 rounded-lg text-sm text-yellow-800">
                            <span className="font-semibold">💡 Подсказка:</span> {node.data.hint}
                          </div>)}
                        {/* Answer input placeholder */}
                        {(node.type === 'TEXT' || node.type === 'CODE') && (<div>
                            <input type="text" placeholder={node.type === 'CODE' ? 'Введите код...' : 'Введите ответ...'} className="w-full p-3 border border-gray-300 rounded-lg text-sm text-gray-800 bg-gray-50" readOnly/>
                            <button className="mt-2 w-full py-2.5 bg-blue-500 text-white rounded-lg text-sm font-semibold">
                              Отправить
                            </button>
                          </div>)}
                        {/* Choice options placeholder */}
                        {node.type === 'CHOICE' && node.data.options && (<div className="space-y-2">
                            {node.data.options.map((opt, idx) => (<div key={idx} className="p-3 border border-gray-200 rounded-lg text-sm text-gray-700 bg-gray-50">
                                {opt}
                              </div>))}
                          </div>)}
                        {/* Points */}
            ...
            }} disabled={nodes.filter((n) => n.type !== 'START' && n.type !== 'FINISH').findIndex((n) => n.id === currentPreviewNode) <= 0} className="px-4 py-2 bg-gray-700 rounded hover:bg-gray-600 disabled:opacity-30 disabled:cursor-not-allowed text-sm">
                  ← Предыдущее
                </button>
            ...
            }} disabled={nodes.filter((n) => n.type !== 'START' && n.type !== 'FINISH').findIndex((n) => n.id === currentPreviewNode) >= nodes.filter((n) => n.type !== 'START' && n.type !== 'FINISH').length - 1} className="px-4 py-2 bg-gray-700 rounded hover:bg-gray-600 disabled:opacity-30 disabled:cursor-not-allowed text-sm">
                  Следующее →
                </button>
              </div>
            </div>
      ...
      {/* Settings Panel */}
      {showSettings && (<div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-background rounded-lg shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[80vh]">
            ...
            <div className="flex-1 overflow-auto p-6 space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" value={gameSettings.totalTime ?? 0} onChange={(e) => setGameSettings((prev) => ({ ...prev, totalTime: parseInt(e.target.value) || 0 }))} className="input-field text-sm" min={0}/>
                  </div>
                  <div>
                    <label className="label text-xs">Лимит подсказок</label>
                    <input type="number" value={gameSettings.hintLimit} onChange={(e) => setGameSettings((prev) => ({ ...prev, hintLimit: parseInt(e.target.value) || 1 }))} className="input-field text-sm" min={1}/>
                  </div>
                  <div>
                    <label className="label text-xs">Очки по умолчанию</label>
                    <input type="number" value={gameSettings.defaultPoints} onChange={(e) => setGameSettings((prev) => ({ ...prev, defaultPoints: parseInt(e.target.value) || 0 }))} className="input-field text-sm"/>
                  </div>
                  <div>
                    <label className="label text-xs">Штраф по умолчанию</label>
                    <input type="number" value={gameSettings.defaultPenalty} onChange={(e) => setGameSettings((prev) => ({ ...prev, defaultPenalty: parseInt(e.target.value) || 0 }))} className="input-field text-sm"/>
                  </div>
                  <div>
                    <label className="label text-xs">Макс. попыток</label>
                    <input type="number" value={gameSettings.maxAttempts} onChange={(e) => setGameSettings((prev) => ({ ...prev, maxAttempts: parseInt(e.target.value) || 1 }))} className="input-field text-sm" min={1}/>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {gameSettings.variables.map((v) => (<div key={v.id} className="flex gap-2 items-start p-2 bg-background/50 rounded border border-border">
                      <div className="flex-1 space-y-1">
                        <input type="text" value={v.name} onChange={(e) => updateVariable(v.id, 'name', e.target.value)} className="input-field text-xs" placeholder="Имя переменной"/>
                        <div className="flex gap-1">
                          <select value={v.type} onChange={(e) => updateVariable(v.id, 'type', e.target.value)} className="input-field text-xs flex-1">
                            <option value="number">Число</option>
                            <option value="string">Строка</option>
                            <option value="boolean">Булево</option>
                          </select>
                          <input type="text" value={String(v.defaultValue)} onChange={(e) => {
                    const val = v.type === 'number' ? parseFloat(e.target.value) || 0 : e.target.value;
                    updateVariable(v.id, 'defaultValue', val);
                }} className="input-field text-xs w-20" placeholder="По умолч."/>
                        </div>
                      </div>
                      <button onClick={() => removeVariable(v.id)} className="text-error hover:text-error/80 text-lg mt-1">
                        ×
                      </button>
                    </div>))}
            ...
            <div className="p-4 bg-gray-800 border-t flex justify-end">
              <button onClick={() => setShowSettings(false)} className="px-4 py-2 bg-gray-600 rounded hover:bg-gray-500 text-sm">
                Закрыть
              </button>
            </div>
          </div>
      ...
      {showTest && (<div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-background rounded-lg shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col h-[80vh]">
            <div className="bg-gray-800 p-4 flex items-center justify-between">
              ...
              <button onClick={() => setShowTest(false)} className="text-gray-400 hover:text-white text-xl">
                ×
              </button>
            </div>
            <div className="flex-1 overflow-auto p-6">
              ...
              <div className="flex items-center justify-between mb-4 p-3 bg-gray-800/50 rounded-lg border border-gray-700">
                <div className="flex items-center gap-4">
                  ...
                  {Object.entries(testVariables).filter(([k]) => k !== 'score').length > 0 && (<div className="flex gap-2 text-xs text-gray-400">
                      {Object.entries(testVariables)
                    .filter(([k]) => k !== 'score')
                    .map(([key, val]) => (<span key={key} className="bg-gray-700 px-2 py-1 rounded">
                            {key}: {String(val)}
                          </span>))}
                    </div>)}
                </div>
              ...
              {testCurrentNodeId && !testFinished && (() => {
                const currentNode = nodes.find((n) => n.id === testCurrentNodeId);
                if (!currentNode)
                    return <p className="text-gray-400">Узел не найден</p>;
                return (<div className="border border-gray-700 rounded-lg p-6 bg-gray-800/50">
                    ...
                    {currentNode.data.question && (<p className="text-gray-300 mb-4 text-lg">{currentNode.data.question}</p>)}

                    {currentNode.data.hint && (<div className="bg-yellow-500/10 border border-yellow-500/30 p-3 rounded mb-4">
                        <span className="text-yellow-400 text-sm">💡 Подсказка: {currentNode.data.hint}</span>
                      </div>)}

                    {/* NPC Dialogue */}
                    {currentNode.type === 'NPC' && currentNode.data.npcDialogues && (<div className="space-y-3 mb-4">
                        {currentNode.data.npcDialogues.map((dialogue, dIdx) => (<div key={dIdx} className="bg-cyan-500/10 border border-cyan-500/30 p-4 rounded-lg">
                            ...
                            <div className="space-y-2 ml-8">
                              {dialogue.options.map((opt, oIdx) => (<button key={oIdx} onClick={() => handleTestAnswer(String(dIdx))} className="w-full text-left p-2 bg-gray-700 hover:bg-gray-600 rounded text-sm text-gray-200 transition-colors">
                    ...
                    {/* CHOICE */}
                    {currentNode.type === 'CHOICE' && currentNode.data.options && (<div className="space-y-2 mb-4">
                        {currentNode.data.options.map((option, idx) => (<button key={idx} onClick={() => handleTestAnswer(String(idx))} className="w-full text-left p-3 bg-gray-700 hover:bg-gray-600 rounded text-sm text-gray-200 transition-colors border border-gray-600">
                            {option}
                          </button>))}
                      </div>)}

                    {/* TEXT / CODE input */}
                    {(currentNode.type === 'TEXT' || currentNode.type === 'CODE') && (<div className="space-y-3 mb-4">
                        <input type="text" placeholder={currentNode.type === 'CODE' ? 'Введите код...' : 'Введите ответ...'} className="input-field text-sm w-full" onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                                handleTestAnswer(e.target.value);
                                e.target.value = '';
                            }
                        }}/>
                    ...
                    {(currentNode.type && ['START', 'BRANCH', 'TIMER', 'PHOTO', 'GPS', 'QR'].includes(currentNode.type)) && (<button onClick={() => handleTestAnswer('continue')} className="btn-primary w-full">
                        ➡️ Продолжить
                      </button>)}
                  </div>);
            })()}
              ...
              {/* Test Finished */}
              {testFinished && (<div className="text-center py-12">
              ...
              <div className="mt-6">
                ...
                <div className="bg-gray-900 rounded-lg p-3 max-h-40 overflow-y-auto space-y-1">
                  {testLog.map((entry, idx) => (<p key={idx} className="text-xs text-gray-400">{entry}</p>))}
                </div>
              </div>
            </div>
            <div className="p-4 bg-gray-800 border-t flex justify-end">
              <button onClick={() => setShowTest(false)} className="px-4 py-2 bg-gray-600 rounded hover:bg-gray-500 text-sm">
                Закрыть
              </button>
            </div>
          </div>
...
//# sourceMappingURL=ScenarioEditor.js.map