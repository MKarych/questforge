...
exports.default = PlayFinishPage;
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-2xl mx-auto">
          <div className="card text-center py-12">
            ...
            {/* Progress */}
            <div className="mb-8">
              <ProgressBar_1.default current={correctAnswers} total={totalTasks} difficulty={correctAnswers / totalTasks > 0.7 ? 'easy' :
            ...
            {/* History */}
            {sessionState.history.length > 0 && (<div className="mb-8">
                <h2 className="text-lg font-semibold mb-4 text-text-primary">
                  📊 Результаты по заданиям
                </h2>
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {sessionState.history.map((entry, index) => (<div key={index} className={`flex items-center justify-between p-3 rounded-lg ${entry.result === 'success' ? 'bg-success/10' :
                    entry.result === 'timeout' ? 'bg-warning/10' : 'bg-error/10'}`}>
                      <div className="flex items-center gap-3">
                        <span className="text-sm font-medium text-text-primary">
                          Задание {entry.nodeId}
                        </span>
                        <span className={`text-xs px-2 py-1 rounded ${entry.result === 'success' ? 'bg-success text-white' :
                    ...
                    entry.result === 'timeout' ? 'Таймаут' : 'Ошибка'}
                      ...
                      <div className="flex items-center gap-2">
                        {entry.score !== undefined && entry.score !== 0 && (<span className={`text-sm font-bold ${entry.score > 0 ? 'text-success' : 'text-error'}`}>
                            {entry.score > 0 ? '+' : ''}{entry.score}
                          </span>)}
                        <span className={`text-xl font-bold ${entry.result === 'success' ? 'text-success' :
            ...
            {/* Share Button */}
            <button onClick={handleShare} className="btn-secondary mb-4 flex items-center gap-2 mx-auto">
              <span>📤</span>
            ...
            <div className="flex gap-4 justify-center flex-wrap">
              <link_1.default href={`/play/${shareLink}`} className="btn-secondary">
                🔄 Играть снова
              </link_1.default>
              <link_1.default href="/games" className="btn-primary">
                🎮 Другие игры
