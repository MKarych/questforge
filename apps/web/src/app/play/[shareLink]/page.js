...
function PlayLobbyPage() {
    ...
    (0, react_1.useEffect)(() => {
        async function loadData() {
            try {
                ...
                const gameResponse = await client_1.apiClient.get(`/games/public/share/${shareLink}`);
    ...
    const handleRegister = async () => {
        ...
        setSuccess(null);
        try {
            const response = await (0, client_1.registerTeam)(game.id, selectedTeamId);
        ...
        <div className="max-w-md mx-auto">
          <div className="card">
            <div className="text-center mb-6">
              ...
              {game && (<div className="flex flex-wrap justify-center gap-3 mt-4 text-sm text-text-muted">
                  <span>📍 {game.city}</span>
            ...
            {success ? (<div className="text-center">
                ...
                <link_1.default href={`/play/${shareLink}/lobby`} className="btn-primary w-full">
                  Перейти в лобби
                </link_1.default>
              </div>) : (<>
                {myTeams.length > 0 ? (<div className="space-y-4">
                    <div>
                      <label className="label">Выберите команду</label>
                      <select value={selectedTeamId} onChange={(e) => setSelectedTeamId(e.target.value)} className="input-field">
                    ...
                    <button onClick={handleRegister} disabled={registering || !selectedTeamId} className="btn-primary w-full disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    <link_1.default href="/teams/create" className="btn-primary w-full">
                      Создать команду
                    </link_1.default>
                  </div>)}
              </>)}
            ...
            <div className="mt-6 pt-6 border-t border-border">
              <p className="text-sm text-text-muted text-center">
                Уже играете?{' '}
                <link_1.default href={`/play/${shareLink}/existing`} className="text-primary hover:text-primary-hover">
                  Продолжить игру
                </link_1.default>
              </p>
            </div>
          ...
          <div className="mt-6 text-center">
            <link_1.default href="/games" className="text-text-secondary hover:text-text-primary text-sm">
