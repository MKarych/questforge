...
function GamePage() {
    ...
    (0, react_1.useEffect)(() => {
        async function loadGame() {
            try {
                const response = await (0, client_1.getGame)(gameId);
    ...
    const handlePublish = async () => {
        if (!game)
            return;
        if (!confirm('Вы уверены, что хотите опубликовать игру?'))
            return;
        setActionLoading('publish');
        try {
            const updated = await (0, client_1.publishGame)(gameId);
    ...
    const handleDelete = async () => {
        ...
        try {
            await (0, client_1.removeGame)(gameId);
    ...
    const handleSaveDraft = async () => {
        ...
        try {
            const updated = await (0, client_1.updateGame)(gameId, { status: 'CREATED' });
            setGame(updated.data);
        }
        catch (err) {
    ...
    if (error || !game) {
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="card border-error text-center py-12">
            ...
            <link_1.default href="/organizer/games" className="btn-primary">
              ← Назад к играм
            </link_1.default>
          </div>
        </div>
    ...
    return (<div className="min-h-screen">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-6">
          <link_1.default href="/organizer/games" className="text-text-secondary hover:text-text-primary text-sm">
            ← Назад к играм
          </link_1.default>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="card">
              ...
              {game.imageUrl && (<div className="mb-6">
                  <img src={game.imageUrl} alt={game.title} className="w-full h-64 object-cover rounded-lg"/>
                </div>)}
            ...
            <div className="card">
              ...
              {game.scenario ? (<div className="space-y-2">
                ...
                </div>) : (<p className="text-text-secondary">Сценарий не привязан</p>)}
            </div>

            {/* Reviews */}
            <div className="card">
              <h2 className="text-lg font-semibold text-text-primary mb-4">Отзывы ({game.reviewsCount})</h2>
              {!game.reviews || game.reviews.length === 0 ? (<p className="text-text-secondary">Отзывов пока нет</p>) : (<div className="space-y-4">
                  {game.reviews.map((review) => (<div key={review.id} className="border-b border-border pb-3 last:border-0">
                      ...
                      <div className="flex items-center gap-1 mb-1">
                        {Array.from({ length: review.rating }).map((_, i) => (<span key={i} className="text-warning">★</span>))}
                      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="card">
              ...
              <div className="space-y-3">
                ...
                {game.publishedAt && (<div className="flex justify-between">
            ...
            <div className="card">
              ...
              <div className="flex items-center gap-2">
                <input type="text" value={`${typeof window !== 'undefined' ? window.location.origin : ''}/game/${game.shareLink}`} readOnly className="input-field flex-1 text-sm"/>
                <button className="btn-secondary text-sm px-3" onClick={() => {
            ...
            <div className="card">
              ...
              <div className="flex flex-col gap-2">
                <link_1.default href={`/organizer/games/${gameId}/edit`} className="btn-secondary text-center">
                  Редактировать
                </link_1.default>
                {game.status === 'CREATED' && (<button className="btn-primary text-center" onClick={handlePublish} disabled={actionLoading === 'publish'}>
                    {actionLoading === 'publish' ? '...' : 'Опубликовать'}
                  </button>)}
                {game.status !== 'PUBLISHED' && game.status !== 'FINISHED' && (<button className="btn-secondary text-center text-error hover:border-error" onClick={handleDelete} disabled={actionLoading === 'delete'}>
                    {actionLoading === 'delete' ? '...' : 'Удалить'}
                  </button>)}
                {game.status !== 'PUBLISHED' && (<button className="btn-secondary text-center" onClick={handleSaveDraft} disabled={actionLoading === 'save'}>
            ...
            <div className="card">
              ...
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center overflow-hidden flex-shrink-0">
                  {game.organizer?.avatarUrl ? (<img src={game.organizer.avatarUrl} alt={game.organizer.name} className="w-full h-full object-cover"/>) : (<span className="text-lg font-semibold text-primary">
