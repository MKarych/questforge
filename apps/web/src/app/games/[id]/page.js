...
exports.default = GameDetailsPage;
...
const ImageModal_1 = __importDefault(require("@/components/ui/ImageModal"));
...
function GameDetailsPage() {
    ...
    const [selectedTeamId, setSelectedTeamId] = (0, react_1.useState)('');
    ...
    (0, react_1.useEffect)(() => {
        async function loadGame() {
            try {
                const response = await (0, client_1.getPublicGame)(gameId);
                ...
                try {
                    ...
                    if (teamsResponse.data && Array.isArray(teamsResponse.data)) {
                        ...
                        if (lastTeamId && teamsResponse.data.some((t) => t.id === lastTeamId)) {
    ...
    const handleJoin = async (e) => {
        ...
        try {
            ...
            const sessionResponse = await (0, client_1.startSession)({
                gameId: game.id,
    ...
    return (<div className="min-h-screen">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          ...
          <div className="lg:col-span-2">
            {game.imageUrl && (<button type="button" onClick={() => setModalOpen(true)} className="relative w-full h-64 mb-6 rounded-xl overflow-hidden cursor-pointer text-left">
                <image_1.default src={game.imageUrl} alt={game.title} fill className="object-cover hover:scale-105 transition-transform duration-300" quality={85}/>
              </button>)}
            {!game.imageUrl && (<div className="relative w-full h-64 mb-6 rounded-xl overflow-hidden bg-gradient-to-br from-primary/30 to-surface-elevated flex items-center justify-center">
                <image_1.default src={DEFAULT_LOGO} alt={game.title} fill className="object-contain p-8" quality={100} unoptimized/>
              </div>)}
            ...
            <div className="flex flex-wrap gap-4 mb-6">
              ...
              {game.averageRating > 0 && (<div className="card flex items-center gap-2">
                  <span className="text-2xl">⭐</span>
            ...
            {/* Reviews */}
            {game.reviews.length > 0 && (<div className="mt-8">
                <h2 className="text-xl font-semibold mb-4 text-text-primary">Отзывы</h2>
                <div className="space-y-4">
                  {game.reviews.map((review) => (<div key={review.id} className="card">
          ...
          <div className="lg:col-span-1">
            <div className="card sticky top-24">
              ...
              {success ? (<div className="text-center">
                ...
                </div>) : myTeams.length > 0 ? (<form onSubmit={handleJoin} className="space-y-4">
                  <div>
                    <label className="label">Выберите команду</label>
                    <select value={selectedTeamId} onChange={(e) => setSelectedTeamId(e.target.value)} className="input-field" required>
                      <option value="">-- Выберите команду --</option>
                      {myTeams.map((team) => (<option key={team.id} value={team.id}>
                  ...
                  <button type="submit" disabled={joining || !selectedTeamId} className="btn-primary w-full disabled:opacity-50 disabled:cursor-not-allowed">
                    {joining ? 'Присоединение...' : 'Начать игру'}
                  </button>

                  <div className="text-center">
                    <link_1.default href="/teams/create" className="text-primary hover:text-primary-hover text-sm">
                  ...
                  <link_1.default href="/teams/create" className="btn-primary w-full inline-block">
                    Создать команду
                  </link_1.default>
                </div>)}
      ...
      {modalOpen && game?.imageUrl && (<ImageModal_1.default src={game.imageUrl} alt={game.title} onClose={() => setModalOpen(false)}/>)}
    </div>);
}
